#!/bin/bash
#PBS -N VELOC_retest
#PBS -l select=40:ncpus=16:mpiprocs=16:mem=62gb:phase=8a,walltime=03:00:00,place=scatter
...
mkdir -p $TMPDIR/logs
declare -a NUM_NODES=(1 4 16 32 40)
...
do
    ...
    do
	    ...
	    rm -rf $TMPDIR/veloc/*
            ...
            mpirun -n ${procs} -npernode ${PPN} -x MPICH_MAX_THREAD_SAFETY=multiple -x LD_PRELOAD=/lib64/libSegFault.so ./benchmark veloc.cfg $stats_file $strat $tol $procs
...
mv $HOME/ECE8730/bash_scripts/$PBS_JOBNAME* $LOG_PATH
